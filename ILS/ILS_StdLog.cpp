...
StdLogger::StdLogger(std::ostream& l_out,
	std::ostream& w_out,
	std::ostream& e_out)
	: log_out(&l_out, [](std::ostream*) {})
	, wrn_out(&w_out, [](std::ostream*) {})
	, err_out(&e_out, [](std::ostream*) {})
{
	...
	, err_out(&e_out, [](std::ostream*) {})
{
	using namespace std;
	if (l_out_file != "") {
		log_out = std::make_shared<std::ofstream>(l_out_file.c_str(), mode | std::ios_base::out);
	}
	BaseLogger::onLogStart(true, wrn_out != log_out, err_out != wrn_out && err_out != log_out);
}
	...
	, err_out(&e_out, [](std::ostream*) {})
{
	...
	if (w_out_file != "") {
		wrn_out = std::make_shared<std::ofstream>(w_out_file.c_str(), mode | std::ios_base::out);
	}
	BaseLogger::onLogStart(true, wrn_out != log_out, err_out != wrn_out && err_out != log_out);
}
	...
	, err_out{}
{
	...
	if (w_out_file == l_out_file) {
		wrn_out = log_out;
	}
	else if (w_out_file != "") {
		wrn_out = std::make_shared<std::ofstream>(w_out_file.c_str(), mode | std::ios_base::out);
	}
	if (e_out_file == l_out_file) {
		err_out = log_out;
	}
	else if (e_out_file == w_out_file) {
		err_out = wrn_out;
	}
	else if (e_out_file != "") {
		err_out = std::make_shared<std::ofstream>(e_out_file.c_str(), mode | std::ios_base::out);
	}
	BaseLogger::onLogStart(true, wrn_out != log_out, err_out != wrn_out && err_out != log_out);
}
